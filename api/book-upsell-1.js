// POST /api/book-upsell-1
// One-click $27 charge for Build Your AI CEO Video Course — book funnel upsell
// Uses same customer + payment method from original book bundle checkout PaymentIntent
// NOTE: metadata[product] = 'build-your-ai-ceo' — same key as direct course purchase for unified access
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. Charge $27 using the same payment method
    const upsellBody = new URLSearchParams({
      ...
      description:      'Build Your AI CEO Video Course — Book Funnel Upsell',
    ...
    if (upi.error || (upi.status !== 'succeeded' && upi.status !== 'processing')) {
      console.error('Book upsell-1 charge error:', upi.error || upi.status);
